 ...
 * @generated
 */
public class ConferenceValidationProvider {
	...
	public static void runWithConstraints(TransactionalEditingDomain editingDomain, Runnable operation) {
		...
		if (editingDomain != null) {
			try {
				...
				ConferenceDiagramEditorPlugin.getInstance().logError("Validation failed", e); //$NON-NLS-1$
			}
		} else {
			task.run();
		}
	...
	static boolean isInDefaultEditorContext(Object object) {
		...
		if (object instanceof View) {
			return constraintsActive
					&& ConferenceEditPart.MODEL_ID.equals(ConferenceVisualIDRegistry.getModelID((View) object));
		}
		return true;
	}
